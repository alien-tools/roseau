package io.github.alien.roseau.diff.rules.breaking;

import io.github.alien.roseau.api.model.TypeDecl;
import io.github.alien.roseau.diff.changes.BreakingChangeKind;
import io.github.alien.roseau.diff.rules.TypeRule;
import io.github.alien.roseau.diff.rules.TypeRuleContext;

public class TypeNowSealed implements TypeRule<TypeDecl> {
	@Override
	public void onMatched(TypeDecl oldType, TypeDecl newType, TypeRuleContext ctx) {
		if (!oldType.isSealed() && newType.isSealed() &&
			ctx.v1().analyzer().canBeDirectlySubtyped(oldType)) {
			ctx.builder().typeBC(BreakingChangeKind.TYPE_NOW_SEALED, oldType);
		}
	}
}
