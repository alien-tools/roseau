	...
	public void onMatchedExecutable(ExecutableDecl oldExecutable, ExecutableDecl newExecutable, MemberRuleContext ctx) {
		...
		if (paramsCount1 > paramsCount2 && (isOverridable || paramsCount1 > 1)) {
			oldExecutable.getFormalTypeParameters().subList(paramsCount2, paramsCount1)
				.forEach(ftp ->
					ctx.builder().memberBC(BreakingChangeKind.METHOD_FORMAL_TYPE_PARAMETERS_REMOVED, ctx.oldType(), oldExecutable, newExecutable,
		...
		if (paramsCount1 > 0 && paramsCount1 < paramsCount2) {
			newExecutable.getFormalTypeParameters().subList(paramsCount1, paramsCount2)
				.forEach(ftp ->
					ctx.builder().memberBC(BreakingChangeKind.METHOD_FORMAL_TYPE_PARAMETERS_ADDED, ctx.oldType(), oldExecutable, newExecutable,
			...
			if (i < paramsCount2) {
				...
				if (isOverridable) { // Invariant
					if (!new HashSet<>(bounds1).equals(new HashSet<>(bounds2))) {
						ctx.builder().memberBC(BreakingChangeKind.METHOD_FORMAL_TYPE_PARAMETERS_CHANGED, ctx.oldType(), oldExecutable, newExecutable,
